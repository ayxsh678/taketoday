...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: deck,
    openGraph: {
      title, description: deck, type: "article", siteName: SITE.name,
      publishedTime: publishedAt, modifiedTime: updatedAt ?? publishedAt,
      authors: [author.name], section: category,
      images: [{ url: ogImage, width: 1200, height: 630, alt: title }],
    },
    twitter: { card: "summary_large_image", title, description: deck, images: [ogImage] },
  };
}
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const newsArticleLd = {
    ...
    headline: title, description: deck,
    datePublished: publishedAt, dateModified: updatedAt ?? publishedAt,
    url: articleUrl, image: ogImage,
    author: { "@type": author.type === "Person" ? "Person" : "Organization", name: author.name },
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: category, item: abs(`/category/${category.toLowerCase()}`) },
