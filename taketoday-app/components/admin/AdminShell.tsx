...
export function AdminShell({
  ...
  const filteredNav = q
    ? adminNav.filter((n) => n.label.toLowerCase().includes(q) || n.description.toLowerCase().includes(q))
    : adminNav;
  ...
  return (
    ...
    >
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 flex w-56 flex-col transition-transform duration-200 lg:translate-x-0",
      ...
      >
        ...
        >
          ...
          >
            <span
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[10px] font-extrabold tracking-tight"
          ...
          <div className="space-y-0.5">
            {adminNav.map((nav) => {
              ...
              return (
                ...
                >
                  <span style={{ color: active ? "var(--adm-accent-blue)" : "var(--adm-text-3)" }}>
        ...
        >
          ...
          >
            <span
              ...
              style={{
                background: "linear-gradient(135deg, var(--adm-accent-blue), var(--adm-accent-purple))",
                color: "#fff",
              }}
            >
              {initials}
      ...
      <div className="min-h-screen lg:pl-56">
        {/* header */}
        <header
          className="sticky top-0 z-20 flex h-14 items-center justify-between gap-4 px-4 sm:px-6"
          style={{
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="hidden h-8 items-center gap-2 rounded-md px-3 text-sm transition sm:flex"
              style={{
                background: "var(--adm-surface-2)",
            ...
            >
              ...
              <span
                ...
                style={{ border: "1px solid var(--adm-border)", color: "var(--adm-text-3)" }}
              >
        ...
        <main className="mx-auto w-full max-w-site px-4 py-6 sm:px-6">
          {children}
        </main>
      </div>
      ...
      {commandOpen && (
        <div
          ...
          style={{ background: "var(--adm-overlay-bg)", backdropFilter: "blur(8px)" }}
          onClick={() => setCommandOpen(false)}
        >
          <div
            className="glass-panel mx-auto mt-20 max-w-lg overflow-hidden rounded-xl"
          ...
          >
            ...
            >
              <Search className="h-4 w-4 shrink-0" style={{ color: "var(--adm-text-3)" }} />
              <input
                autoFocus
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search or jump to…"
                className="h-11 flex-1 bg-transparent text-sm outline-none placeholder:text-(--adm-text-3)"
                style={{ color: "var(--adm-text-1)" }}
              />
              {searchQuery && (
                <button
            ...
            <div className="max-h-80 space-y-2 overflow-y-auto p-2">
              {noResults ? (
                <p className="px-3 py-6 text-center text-sm" style={{ color: "var(--adm-text-3)" }}>
                ...
                <>
                  {filteredNav.length > 0 && (
                    <section>
                      <p className="mb-1 px-3 text-[10px] font-semibold uppercase tracking-widest" style={{ color: "var(--adm-text-3)" }}>
                        Pages
                      </p>
                      {filteredNav.map((nav) => (
                        <Link
                          ...
                          style={{ color: "var(--adm-text-1)" }}
                        >
                          <span style={{ color: "var(--adm-text-3)" }}><nav.icon className="h-4 w-4 shrink-0" /></span>
                          <span className="flex-1">{nav.label}</span>
                          <span className="text-xs" style={{ color: "var(--adm-text-3)" }}>{nav.description}</span>
                        </Link>
                      ))}
                    </section>
                  )}
                  {filteredActions.length > 0 && (
                    <section>
                      ...
                      {filteredActions.map((action) => (
                        ...
                        >
                          <Zap className="h-4 w-4 shrink-0" style={{ color: "var(--adm-accent-amber)" }} />
                          {action}
                        </button>
                      ))}
                    </section>
                  )}
                  <div style={{ borderTop: "1px solid var(--adm-border)" }} className="pt-1">
                    <button
                      type="button"
                      onClick={() => void handleSignOut()}
                      className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors hover:bg-red-500/10"
                      style={{ color: "var(--adm-text-2)" }}
                    >
